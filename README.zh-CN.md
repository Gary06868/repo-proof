...
RepoProof 本质上是一个普通 skill 目录。最可靠的方式，是把这个仓库 clone 到你正在使用的 agent 会读取的 skills 目录。
...
如果你的环境支持 Open Agent Skills 或 `gh skill`，可以安装到等价的 skill 目录，或运行：

```sh
gh skill install Gary06868/repo-proof repo-proof
```

兼容路径包括 `.agents/skills/repo-proof`、`~/.agents/skills/repo-proof`、`.codex/skills/repo-proof`、`~/.codex/skills/repo-proof`、`.claude/skills/repo-proof`、`~/.claude/skills/repo-proof`、`.cursor/skills/repo-proof`、`.github/skills/repo-proof` 和 `.gemini/skills/repo-proof`。
